...
**Files:**
- Modify: `src/krater/config.py` (drop `inbox_bot_token`, `owner_telegram_id`)
- Rewrite: `src/krater/inbox.py`
- Modify: `src/krater/notify.py` (add `LogNotifier`)
- Modify: `src/krater/app.py`, `src/krater/cli.py`, `pyproject.toml`, `.env.example`, `README.md`, `Dockerfile`
- Delete: `src/krater/telegram_notify.py`, `tests/test_telegram_notify.py`, `tests/test_app.py`
...
- [ ] **Step 3: Rewrite `src/krater/inbox.py`**
...
```
(imports: `from krater.inbox import Inbox`, `from krater.youtube import YouTubeEntry, YouTubeError`). Fix the other two `Settings(...)`/`create_app(...)` calls in that file the same way. Add the test:
...
- `git rm src/krater/telegram_notify.py tests/test_telegram_notify.py tests/test_app.py`.
...
- Modify: `src/krater/app.py` (call `migrate_legacy_data_dir` first thing in `run`)
...
- Produces:
  ...
  LEGACY_DATA_DIR = Path("~/.config/krater")
  def default_data_dir() -> Path        # ~/Library/Application Support/Krater on darwin, else LEGACY_DATA_DIR
  ...
  def migrate_legacy_data_dir(settings: Settings) -> bool       # moves ~/.config/krater to the new default once
  ```
...
def test_migrate_legacy_data_dir(tmp_path: Path, monkeypatch):
    ...
    legacy.mkdir()
    (legacy / "krater.sqlite").write_text("db")
    new = tmp_path / "new"
    monkeypatch.setattr("krater.config.LEGACY_DATA_DIR", legacy)
    ...
    assert (new / "krater.sqlite").read_text() == "db" and not legacy.exists()
...
def migrate_legacy_data_dir(settings: Settings) -> bool:
    """Move ~/.config/krater to the Mac-native folder on first launch. Only when the data folder is
...
Replace the bullet `~/.config/krater/ — sqlite database…` with `~/Library/Application Support/Krater/` (macOS; `~/.config/krater/` elsewhere, and an existing folder there is moved on first launch) — sqlite database, `settings.json`, the Telethon session, tmp downloads, and spectrogram PNGs.` Mention that `settings.json` holds the library folder and the Telegram api id/hash and that `.env` overrides it.
...
**Files:**
- Modify: `src/krater/store.py`
- Create: `src/krater/events.py`
...
**Files:**
- Delete: `src/krater/web.py`
- Create: `src/krater/web/__init__.py`, `src/krater/web/requests.py`, `src/krater/web/stream.py`, `src/krater/web/library.py`
- Modify: `src/krater/worker.py` (add `retry`)
...
`git rm src/krater/web.py`. `web/__init__.py`:
...
**Files:**
- Modify: `src/krater/web/library.py`
...
def test_reveal_is_confined_to_app_folders(tmp_path: Path):
    opened = []
    app, store, settings = make(tmp_path)
    from krater.web import library as lib_module  # noqa: F401  (ensures the module is importable)
    app2, _, settings2 = make(tmp_path)
    # rebuild with an injected opener
    from krater.web.library import router
    from fastapi import FastAPI
    api = FastAPI()
    api.include_router(router(store, settings, {"setup_done": False}, __import__("krater.web", fromlist=["Bundles"]).Bundles(store),
...
**Files:**
- Create: `web/package.json`, `web/vite.config.ts`, `web/tsconfig.json`, `web/index.html`, `web/src/main.tsx`, `web/src/App.tsx` (placeholder that renders "Krater"), `web/src/theme.css`, `web/src/api.ts`, `web/src/api.test.ts`, `web/src/setupTests.ts`
...
```
`web/src/App.tsx` (placeholder for this task): `export default function App() { return <div className="app">Krater</div> }`.
...
export default function Shell({ tab, onTab, telegramAuthorized, banner, sidebarExtra, children }: { tab: Tab; onTab: (t: Tab) => void; telegramAuthorized: boolean; banner?: ReactNode; sidebarExtra?: ReactNode; children: ReactNode }) {
  return (
    <div className="app">
      <div className="titlebar">Krater</div>
      {banner}
...
**Interfaces:**
- `SettingsPage({ live })`: title "Settings" (20 px 700); four cards max-width 640: "Library folder" (mono path, "Change" → inline text field prefilled with the path + "Save"/"Cancel"; on save `api.saveSettings(path)` then `live.setSettings(result)`; an `ApiError` message shows in red under the field), "Telegram" (green dot "Connected" / amber dot "Signed out"; the masked phone arrives in Task 14), "App version" ("Krater {version}"), "App data" (mono `data_dir`, "Show in Finder" → `api.reveal(data_dir)`).
...
const live = { settings: { library_root: '/Users/me/Music/DJ Library', data_dir: '/Users/me/Library/Application Support/Krater', version: '0.1.0', telegram_configured: true },
...
export default function SettingsPage({ live }: { live: Live }) {
  ...
  return (
    <div className="scroll settings">
      ...
      <div className="card scard"><div className="body"><div className="k">App version</div><div className="v">Krater {s.version}</div></div></div>
...
**Files:**
- Create: `src/krater/web/telegram.py`
- Modify: `src/krater/web/__init__.py` (accept `login: TelegramLogin | None`), `src/krater/app.py`
- Test: `tests/test_web.py` (append)
...
- `SetupShell({ step, title, children })`: centred 760×600 panel (in the browser: `max-width: 760px; min-height: 600px; margin: 6vh auto`), title bar text "Welcome to Krater", the mono stepper `1 Folder · 2 Telegram · 3 Ready` (current amber, done green with ✓, future faint).
...
export default function SetupShell({ step, children, onBack }: { step: 1 | 2 | 3; children: ReactNode; onBack?: () => void }) {
  return (<div className="setup"><div className="setup-win">
    <div className="titlebar">Welcome to Krater{onBack && <button className="btn-link" style={{ position: 'absolute', right: 16 }} onClick={onBack}>Back</button>}</div>
...
- [ ] **Step 2:** Run `uv run crate start` against the real data folder (after the migration moved it to `~/Library/Application Support/Krater`). Paste one track link and one playlist link; watch rows move through the steps live without reloading; choose a version on a review row; open a rejection's "See why"; "Show in Finder" opens Finder; Library shows the playlist and its M3U8 card.
