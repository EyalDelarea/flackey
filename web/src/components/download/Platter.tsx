...
/** The transfer's percentage drawn as the app's own icon: a record spinning on a platter, its rim filling
    ...
    A null `pct` is every state with no proportion to draw yet -- queued at the peer, or sending before the
    size is known. The rim sweeps a short arc instead of filling, the same signal the old bar's `.waiting`
    state gave. There is no number to announce either, so the row's own `label` stands in: it is the one
    string that tells those two states apart, and inventing a second one here got it wrong. */
