...
| Transfer platter | 44px record in the right rail beside Stop: track `--platter-track` and arc `--accent` at 3.2, their inner edge tucked under the disc's own edge so ring and record are one object, one groove at 1, percentage at 13/600 tabular in `--platter-ink` straight on the vinyl. The arc is the only accent. It holds still while it has a number — motion is reserved for the indeterminate states, which drop the number, grey the arc and sweep it at 1.9s |
